...
async function checkAbortStatus(job) {
  const abortStatus = _jobAborted[job.id];
  if (abortStatus == true) {
    throw new Error("Job is Aborted");
  }
}
...
function prepTransfers(
  ...
  const native_xfers = global_tx.filter((tx) => tx.value > 0);
  for (tx of native_xfers) {
...
function getRefVault(wallet_vault, ref_assets) {
  ...
  if (wallet_vault.name == "Governance" || wallet_vault.name == "Locked") {
    ref_vault = ref_assets_chain_protocol.find(
      (ref) => ref.deposit_address == wallet_vault.pool.id.toLowerCase()
    );
  }
...
function getLPToken(pool, global_token_info_debank) {
  ...
  let ref_vault = REF_ASSETS.find(
    ...
    (ref) => ref.key == pool.key && ref.deposit_address == pool.pool.id
  );
  if (!ref_vault)
    //Rari: match only key
  ...
  for (const complex_protocol_item of complex) {
    ...
    for (const pool of portfolio_item_list) {
      ...
      if (lp_token_debank) {
        ...
        const lp_token_debank_index = global_token_info_debank.findIndex(
          (token) => token.id == lp_token_debank.id
        );
        global_token_info_debank[lp_token_debank_index].used = true;
        pool.id = lp_token_debank.id;
        const lp_token_moralis = global_balances.find(
          (token) => token.token_address == pool.pool.id
        );
        pool.raw_amount =
        ...
        pool.id = ref_vault?.deposit_tokens?.at(0)?.address?.toLowerCase() || null;
...
function add_vault_transfers(
  ...
  for (const complex_protocol_item of complex) {
    for (const pool of complex_protocol_item.portfolio_item_list) {
      if (pool.name == "Liquidity Pool") lp_tokens.push(pool.pool.id);
    }
  }
  ...
  const footnote = [
    {
      ...
      currentTxCount: global_tx.length,
...
async function makeTokenResult(
  ...
  let token_result = {
    ...
    reward_token_list: wallet_position.detail?.reward_token_list?.map((item) => {
                        return {
                          symbol: item.symbol,
                          amount: item.amount,
                          price: item.price
    ...
    supply_token_list: wallet_position.detail?.supply_token_list?.map((item) => {
                        return {
                          symbol: item.symbol,
                          amount: item.amount,
                          price: item.price
...
async function getTokenCostBasis(
  ...
  let token_cost = 0,
    ...
    newHistory = [];
  let sTime;
  let { price, token_info } = await getTokenPrice(
    chain,
    token,
    blockheight,
    global_token_info_debank,
...
/*
    //Get token price
  let price = null, token_info=null;
    ...
    if (hierarchy_level > 1 && !price && !token_info?.is_core) {
      ...
      if (wrapped_token) {
        ...
        if (MODE_SETTING === "dev") {
          console.log(
            `   Wrapped: ${token_info?.name || token}, Underlying: ${wrapped_token?.symbol
  ...
  */
  ...
  if (FAST_MODE == true && token_transactions.length > 0) {
    sTime = new Date().getTime();
    /*nativePrice = await getOldTokenPrice(
      chain,
    ...
    let { price: nativePrice_ } = await getTokenPrice(
      chain,
      chainCoins[chain].address,
      null,
      global_token_info_debank,
  ...
  for (let i = 0; i < token_transactions.length; i++) {
    ...
    if (FAST_MODE) {
      ...
      nativePrice = nativePrice_;
      /*
      native_price = await getOldTokenPrice(
        chain,
        chainCoins[chain].address,
        blockheight,
        global_token_info_debank
