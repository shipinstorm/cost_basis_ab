...
const walletCostBasisForTest = async (req, res) => {
  ...
  const jobId = `${wallet}${jobChain ? "_" + jobChain : ""}${jobProtocol ? "_" + jobProtocol : ""}`;
  const cache = work.getHistory(jobId);
...
const cancelWalletCostBasis = async (req, res) => {
  const wallet = req.params.id.toLowerCase();
  const jobChain = req.params.chain;
  const jobProtocol = req.params.protcol_id;
  const jobId = `${wallet}${jobChain ? "_" + jobChain : ""}${jobProtocol ? "_" + jobProtocol : ""}`;
...
const walletStatus = async (req, res) => {
  ...
  if (total > 0 && flag == true) {
    ...
    progress = Math.floor((cnt / status.total) * 100);
  }
