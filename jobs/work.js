...
async function workAsync(job) {
  ...
  //console.log("Hello", job.aborted)
  services._jobAborted[job.id] = false;
  ...
  const end = new Date();
  if ( result == null) {
    deleteHistory(job.id);
  } else {
    _history[job.id] = { time: end.getTime(), result };    
  }
  deleteJobAborted(job.id);
...
function setJobAborted(id) {
  if (typeof(services._jobAborted[id]) === "undefined")
    return;
  services._jobAborted[id] = true;
}

function deleteJobAborted(id) {
  delete services._jobAborted[id];
}
